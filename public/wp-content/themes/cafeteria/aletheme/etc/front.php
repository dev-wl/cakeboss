...
function ale_enqueue_styles() {
    ...
    wp_register_style( 'scheme1', THEME_URL . '/css/colors/scheme1.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'scheme2', THEME_URL . '/css/colors/scheme2.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'scheme3', THEME_URL . '/css/colors/scheme3.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'scheme4', THEME_URL . '/css/colors/scheme4.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'scheme5', THEME_URL . '/css/colors/scheme5.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( 'scheme6', THEME_URL . '/css/colors/scheme6.css', array(), ALETHEME_THEME_VERSION, 'all');

    wp_enqueue_style('aletheme_general_css');

    //Check if User have styles cookie, if no show the color from Theme Options.
    if(!isset($_COOKIE["cafeteriacolor"])) {
        wp_enqueue_style(ale_get_option('colscheme'));
    } else {
        wp_enqueue_style($_COOKIE["cafeteriacolor"]);
    }
...
function ale_enqueue_scripts() {
    ...
    wp_register_script( 'InitHome', THEME_URL . '/js/libs/InitHome.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'InitDefault', THEME_URL . '/js/libs/InitDefault.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'InitOpenGallery', THEME_URL . '/js/libs/InitOpenGallery.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'InitAbout', THEME_URL . '/js/libs/InitAbout.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'InitOpenMenu', THEME_URL . '/js/libs/InitOpenMenu.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'InitStoryOpen', THEME_URL . '/js/libs/InitStoryOpen.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    ...
    wp_enqueue_script( 'InitDefault' );

    if(is_page_template('page-home.php')){
        wp_enqueue_script( 'InitHome' );
        wp_enqueue_script( 'jquery.isotope.min' );
    }

    if(is_singular('gallery')){
        wp_enqueue_script( 'InitOpenGallery' );
    }

    if(is_singular('menu')){
        wp_enqueue_script( 'InitOpenMenu' );
    }

    if(is_singular('post') or is_singular('events')){
        wp_enqueue_script( 'InitStoryOpen' );
    }

    if(is_page_template('template-about.php')){
        wp_enqueue_script( 'InitAbout' );
    }
        ...
        <div class="cf">
            ...
            <div class="other">
                <div class="comment-info">
                    <h4 class="firstfont"><?php printf( __('%1$s at %2$s','aletheme'), get_comment_date(),  get_comment_time()) ?> / <span class="name"><?php printf(__('%s','aletheme'), get_comment_author_link()) ?></span> / <?php _e('Write','aletheme');?>:</h4>
                </div>
